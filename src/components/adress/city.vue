...
<template>
  ...
  <section class="showChose" v-show="showChose">
    <section class="address">
      ...
      <ul class="ul_city">
        <li class="addList" style="margin-left: 0;padding-left: 0;text-indent: 0.32rem" v-for="(v,k) in info" @click="getProvinceId(v.id, v.name, k)" v-show="showProvince" :class="v.selected ? 'active' : ''">{{v.name}}</li>
        <li class="addList"style="margin-left: 0;padding-left: 0;text-indent: 0.32rem" v-for="(v,k) in showCityList" @click="getCityId(v.id, v.name, k)" v-show="showCity" :class="v.selected ? 'active' : ''">{{v.name}}</li>
        <li class="addList"style="margin-left: 0;padding-left: 0;text-indent: 0.32rem" v-for="(v,k) in showDistrictList" @click="getDistrictId(v.id, v.name, k)" v-show="showDistrict" :class="v.selected ? 'active' : ''">{{v.name}}</li>
      </ul>
    </section>
  </section>
</template>
...
export default {
  ...
  watch: {
    cityStatus: function(newVal,oldVal){
      ...
      this.showChose = true;  //newVal即是chartData
    }
  },
  mounted(){
  },
